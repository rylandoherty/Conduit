...
hexView hv;
    ...
    switch (event.getAction() & MotionEvent.ACTION_MASK) {
      case MotionEvent.ACTION_DOWN:
        ...
        Log.d(TAG, "mode=DRAG");
          ...
          panZoomCalculator.getWin(start);
  ...
  public class PanZoomCalculator {
    ...
    public void doZoom(float scale, PointF zoomCenter) {
      ...
      currentZoom = Math.min(20f, currentZoom);
    ...
    public void reset() {
      ...
      currentPan = new PointF(1f, 1f);
      onPanZoomChanged();
    }
/*
      public float[] getAbsolutePosition(float Ax, float Ay) {

          float x = getAbsolutePosition( currentPan.x, Ax,currentZoom);
          float y = getAbsolutePosition(currentPan.y, Ay,currentZoom);
...
*/
      ...
      public void getWin(PointF start){
         // float[] f = getAbsolutePosition(start.x,start.y);
        float curX;float curY;
          curX =  ((start.x-currentPan.x)/currentZoom);
          curY = ((start.y-currentPan.y)/currentZoom);
              //curX =  start.x -(currentPan.x+(start.x)/currentZoom);
              //curY = start.y -(currentPan.y+(start.y)/currentZoom);
          Log.d(TAG, "start.x" + start.x);
          Log.d(TAG, "start.y" + start.y);
          Log.d(TAG, "currentPan.x=" +currentPan.x);
          Log.d(TAG, "currentPan.y=" +currentPan.y);
          Log.d(TAG, "currentZoom" + currentZoom);
          Log.d(TAG, "curX=" + curX);
          Log.d(TAG, "curY=" + curY);
          /*} else {
              curX = start.x - (currentPan.x/currentZoom) -(start.x/currentZoom );
              curY = start.y - (currentPan.y/currentZoom) -(start.y/currentZoom );
          }
    ...
    */
          ...
          */
          float actualXPos = start.x* currentZoom;
          float actualYPos = currentPan.y* currentZoom - start.y*currentZoom;
          float winWidth = window.getWidth() * currentZoom;
          float winHeight = window.getHeight() * currentZoom;
          Hexagon h = new Hexagon(curX, curY);
          hv = (hexView) child;
          hv.drawIt(h);
